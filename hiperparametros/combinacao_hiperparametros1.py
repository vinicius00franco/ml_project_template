...
melhor_modelo = DecisionTreeClassifier(max_depth=int(best_params[0]),
                                       min_samples_split=int(best_params[1]),
                                       min_samples_leaf=int(best_params[2]))
melhor_modelo.fit(x_treino, y_treino)

score_train_melhor = melhor_modelo.score(x_treino, y_treino)
score_val_melhor = melhor_modelo.score(x_val, y_val)
print(f"Score no treino com os melhores parâmetros: {score_train_melhor}")
print(f"Score na validação com os melhores parâmetros: {score_val_melhor}")
...
plt.axvline(max_index, color='r', linestyle='--', label='Melhor combinação')
